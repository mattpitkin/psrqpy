...
class QueryATNF(object):
    ...
    def __init__(
        ...
        elif self._coord1 is None or self._coord2 is None or self._radius == 0.0:
            # if any are not set then we can't define a boundary
            self._coord1 = self._coord2 = ""
            self._radius = 0.0
        else:
            if not isinstance(self._coord1, str) or not isinstance(self._coord2, str):
                raise ValueError(
                    "Circular boundary centre coordinates must be strings"
                )
            if not isinstance(self._radius, float) and not isinstance(
                self._radius, int
            ):
                raise ValueError("Circular boundary radius must be a float or int")

        self._coord = None
        if self._coord1 and self._coord2 and self._radius != 0.0:
            if params is None:
                params = []

            # set centre coordinate as an astropy SkyCoord
            self._coord = SkyCoord(
                self._coord1, self._coord2, unit=(aunits.hourangle, aunits.deg)
            )

        # set conditions
        condparse = self.parse_conditions(psrtype=psrtype, assoc=assoc, bincomp=bincomp)
        if len(condparse) > 0:
        ...
        try:
            _ = self.get_catalogue(
                path_to_db=loadfromdb, cache=cache, update=checkupdate, version=version
            )
        except IOError:
    ...
    def get_references(self, useads=False, cache=True):
        ...
        if useadst:
            self._refs, self._adsrefs = get_references(useadst, cache=cache)
        else:
        ...
        if colname is not None:
            if colname in self.columns:
                ...
                if column.dtype == self.__dataframe[colname].dtype:
                    ...
                    raise ValueError("Could not update table with supplied column")
            else:
                try:
                    version = self.catalogue.version  # retain version information
                    self.__dataframe = concat(
                        [self.catalogue, Series(column, name=name)], axis=1
                    )
                    self.__dataframe.version = version
                except Exception as e:
                    raise ValueError(
                        "Could not add supplied columns to table: {}".format(str(e))
                    )
        else:
    ...
    def sort(self, sort_attr="JNAME", sort_order="asc", inplace=False):
        ...
        if self.sort_key not in self.columns:
            raise KeyError(
                "Sorting by attribute '{}' is not possible as it "
        ...
        if self._sort_order.lower() in ["asc", "ascending", "up", "^"]:
            ...
            warnings.warn(
                "Unrecognised sort order '{}', defaulting to "
                "'ascending'".format(sort_order),
                UserWarning,
            )
            self._sort_order = "asc"
        ...
        if inplace:
            ...
            _ = self.__dataframe.sort_values(
                self.sort_key, ascending=sortorder, inplace=inplace
            )
            return self.__dataframe
        else:
            return self.__dataframe.sort_values(self.sort_key, ascending=sortorder)
    ...
    def load(self, fname, cleardfonly=False):
        ...
        try:
            ...
            if not cleardfonly:
                ...
                self.__dict__["_QueryATNF__dataframe"] = tmpdict.__dict__[
        ...
        for key in PSR_ALL_PARS:
            if key in thistable.colnames:
                if PSR_ALL[key]["units"]:
                    ...
                    if PSR_ALL[key]["err"] and key + "_ERR" in thistable.colnames:
    ...
    def query_table(
        self, query_params=None, usecondition=True, usepsrs=True, useseparation=True
    ):
        """
        ...
        if not self.empty:  # convert to Table if DataFrame is not empty
            ...
            if not np.all(intab):
                warnings.warn(
                    "Not all request parameters '{}' were in the "
            ...
            if usepsrs and self.psrs is not None:
                ...
                if "JNAME" in dftable.columns:
                    jnames = np.array([psr in self.psrs for psr in dftable["JNAME"]])

                bnames = np.zeros(len(dftable), dtype=bool)
                if "BNAME" in dftable.columns:
                    bnames = np.array([psr in self.psrs for psr in dftable["BNAME"]])
                ...
                if np.any(jnames) and np.any(bnames):
                    ...
                    raise ValueError(
                        "No requested pulsars '{}' were found.".format(self.psrs),
                        UserWarning,
                    )
            ...
            if (
                ...
                and "RAJ" in table.colnames
                and "DECJ" in table.colnames
            ):
                # apply sky coordinate constraint
                catalog = SkyCoord(
                    table["RAJ"], table["DECJ"], unit=(aunits.hourangle, aunits.deg)
                )
        ...
        if isinstance(params, list):
            ...
            for p in params:
                if not isinstance(p, str):
                    raise TypeError(
                        "Non-string value '{}' found in params list".format(p)
                    )
        ...
        if self._query_params is not None:
            ...
            for p in list(self._query_params):
                if p not in PSR_ALL_PARS:
                    warnings.warn(
                        "Parameter '{}' not recognised.".format(p), UserWarning
                    )
    ...
    def pandas(self):
        ...
        if (
            self._coord is not None
            and "RAJD" in dftable.columns
            and "DECJD" in dftable.columns
        ):
            # apply sky coordinate constraint
            catalog = SkyCoord(
                dftable["RAJD"], dftable["DECJD"], unit=(aunits.deg, aunits.deg)
            )
    ...
    def parse_assoc(self):
        ...
        if "ASSOC_REF" not in self.columns:
            ASSOCREFnew = Series(
                np.full(self.catalogue_len, "", dtype="U64"), name="ASSOC_REF"
            )
        else:
            ASSOCREFnew = self.catalogue["ASSOC_REF"].copy()

        ASSOCREFnew[idxassoc] = ASSOCnew[idxassoc].apply(
            lambda x: re.split(r"\]", re.split(r"\[", x)[1])[0]
            if len(re.split(r"\[", x)) > 1
            else np.nan
        )

        # Set values
        ASSOCnew[idxassoc] = ASSOCnew[idxassoc].apply(
    ...
    def parse_type(self):
        ...
        if "TYPE_REF" not in self.columns:
            TYPEREFnew = Series(
                np.full(self.catalogue_len, "", dtype="U64"), name="TYPE_REF"
            )
        else:
            TYPEREFnew = self.catalogue["TYPE_REF"].copy()

        TYPEREFnew[idxtype] = TYPEnew[idxtype].apply(
            lambda x: re.split(r"\]", re.split(r"\[", x)[1])[0]
            if len(re.split(r"\[", x)) > 1
            else np.nan
        )
        ...
        # Set values
        TYPEnew[idxtype] = TYPEnew[idxtype].apply(
            lambda x: re.split(r"\[|,|\(|:", x)[0]
        )
    ...
    def parse_bincomp(self):
        ...
        if "BINCOMP_REF" not in self.columns:
            BINCOMPREFnew = Series(
                np.full(self.catalogue_len, "", dtype="U64"), name="BINCOMP_REF"
            )
        else:
            BINCOMPREFnew = self.catalogue["BINCOMP_REF"].copy()

        BINCOMPREFnew[idxbincomp] = BINCOMPnew[idxbincomp].apply(
            lambda x: re.split(r"\]", re.split(r"\[", x)[1])[0]
            if len(re.split(r"\[", x)) > 1
            else np.nan
        )
        ...
        # Set values
        BINCOMPnew[idxbincomp] = BINCOMPnew[idxbincomp].apply(
            lambda x: re.split(r"\[|,|\(|:", x)[0]
        )
    ...
    def define_dist(self):
        ...
        ONEAU = 149597870.0  # AU in km (from psrcat.h)
        ...
        with np.errstate(invalid="ignore"):
            ...
            DIST[idxa & idxdist & (DIST >= DIST_AMX)] = DIST_AMX[
                idxa & idxdist & (DIST >= DIST_AMX)
            ]
            DIST1[idxa1 & idxdist1 & (DIST1 >= DIST_AMX)] = DIST_AMX[
                (idxa1 & idxdist1 & (DIST1 >= DIST_AMX))
            ]

            DIST[idxa & idxdist & (DIST < DIST_AMX)] = DIST_AMN[
                idxa & idxdist & (DIST < DIST_AMX)
            ]
            DIST1[idxa1 & idxdist1 & (DIST1 < DIST_AMX)] = DIST_AMN[
                (idxa1 & idxdist1 & (DIST1 < DIST_AMX))
            ]
        ...
        idxdist = (
            ~np.isfinite(DIST)
            & ~idxpxgt3
            & np.isfinite(DIST_AMN)
            & np.isfinite(DIST_AMX)
        )
        idxdist1 = (
            ~np.isfinite(DIST)
            & ~idxpxgt3
    ...
    def derived_equatorial(self):
        ...
        sc = BarycentricMeanEcliptic(
            ELONG.values[idx] * aunits.deg, ELAT.values[idx] * aunits.deg
        ).transform_to(ICRS())
        ...
        if np.all([p in self.columns for p in reqpar]):
            ...
            elcosb = el * np.cos(b)
            q = np.arctan2(
                np.cos(l) * np.tan(ecl), np.sin(b) * np.sin(l) * np.tan(ecl) - np.cos(b)
            )
            cq, sq = np.cos(q), np.sin(q)
            eracosdec, edec = np.abs(elcosb * cq - eb * sq), np.abs(
                elcosb * sq + eb * cq
            )
    ...
    def derived_ecliptic(self):
        ...
        # get sky coordinates
        sc = SkyCoord(RAJD[idx].values * aunits.deg, DECJD[idx].values * aunits.deg)
    ...
    def derived_binary(self):
        ...
        if np.all([p in self.columns for p in reqpars]):
            ...
            PB = self.catalogue["PB"].values.copy() * 86400.0  # convert to sec

            idx = np.isfinite(A1) & np.isfinite(PB)

            MASSFN = np.full(self.catalogue_len, np.nan)
            MASSFN[idx] = (
                (4.0 * np.pi**2 / GM_sun.value) * A1[idx] ** 3 / (PB[idx] ** 2)
            )
            ...
            for i, mf in enumerate(MASSFN):
                ...
                try:
                    MINMASS[i] = newton(
                        solfunc, MASS_PSR, args=(SINI_MIN, mf, MASS_PSR), maxiter=1000
                    )
                except RuntimeError:
                    MINMASS[i] = np.nan
                try:
                    MEDMASS[i] = newton(
                        solfunc, MASS_PSR, args=(SINI_MED, mf, MASS_PSR), maxiter=1000
                    )
                except RuntimeError:
                    MEDMASS[i] = np.nan
                try:
                    UPRMASS[i] = newton(
                        solfunc, MASS_PSR, args=(SINI_90, mf, MASS_PSR), maxiter=1000
                    )
                except RuntimeError:
                    UPRMASS[i] = np.nan
            ...
            if np.all([p in self.columns for p in reqpars]):
                ...
                # convert to seconds
                PBERR = self.catalogue["PB_ERR"].values.copy() * 86400.0

                idx = np.isfinite(MASSFN) & np.isfinite(A1ERR) & np.isfinite(PBERR)

                MASSFN_ERR = np.full(self.catalogue_len, np.nan)
                MASSFN_ERR[idx] = MASSFN[idx] * np.sqrt(
                    (3.0 * A1ERR[idx] / A1[idx]) ** 2
                    + (2.0 * PBERR[idx] / PB[idx]) ** 2
                )
        ...
        if np.all([p in self.columns for p in reqpars]):
            ...
            idxn = idx & (ECCnew != 0.0)
            OMnew[idxn] = np.arctan2(EPS1[idxn], EPS2[idxn]) * 180.0 / np.pi

            with np.errstate(invalid="ignore"):
                OMnew = np.mod(OMnew + 360.0, 360.0)  # make sure angles are positive
            ...
            if np.all([p in self.columns for p in reqpars]):
                ...
                idxn = idx & (
                    np.isfinite(EPS1ERR) & np.isfinite(EPS2ERR) & (ECCnew != 0.0)
                )

                OMERRnew[idxn] = (
                    (
                        np.sqrt(
                            (EPS2[idxn] * EPS1ERR[idxn]) ** 2
        ...
        if np.all([p in self.columns for p in reqpars]):
            ...
            if np.all([p in self.columns for p in reqpars]):
                ...
                EPS1ERRnew[idxn] = np.abs(EPS1new[idxn]) * np.sqrt(
                    (ECCERR[idxn] / ECC[idxn]) ** 2
                    + (
                        np.abs(np.cos(np.deg2rad(OM[idxn])))
                        * np.deg2rad(OMERR[idxn])
                        / np.abs(np.sin(np.deg2rad(OM[idxn])))
                    )
                    ** 2
                )
                EPS2ERRnew[idxn] = np.abs(EPS2new[idxn]) * np.sqrt(
                    (ECCERR[idxn] / ECC[idxn]) ** 2
                    + (
                        np.abs(np.sin(np.deg2rad(OM[idxn])))
                        * np.deg2rad(OMERR[idxn])
        ...
        if np.all([p in self.columns for p in reqpars]):
            ...
            PB = self.catalogue["PB"].values.copy() * 86400.0
            ECC = self.catalogue["ECC"].values.copy()
            ...
            MINOMDOT[idx] = (
                3.0
                * (2.0 * np.pi / PB[idx]) ** (5.0 / 3.0)
                * ((MASS_PSR + MINMASS[idx]) * 4.925490946e-6) ** (2.0 / 3.0)
                / (1.0 - ECC[idx] ** 2)
            )
            MINOMDOT[idx] = np.rad2deg(MINOMDOT[idx]) * 86400.0 * 365.25
    ...
    def derived_pb(self):
        ...
        if "FB0_ERR" in self.columns:
            ...
            PBERRnew[idx] = FB0ERR[idx] * PBnew[idx] ** 2 * 86400.0
    ...
    def derived_pbdot(self):
        ...
        if np.all([p in self.columns for p in reqpars]):
            ...
            PBDOTERRnew[idx] = np.sqrt(
                (PB[idx] ** 2 * FB1ERR[idx]) ** 2
                + (2.0 * PB[idx] ** 3 * FB1[idx] * FB0ERR[idx]) ** 2
            )
    ...
    def derived_fb0(self):
        ...
        if "PB_ERR" in self.columns:
            ...
            FB0ERRnew[idx] = PBERR[idx] * (FB0new[idx] ** 2) * 86400.0
    ...
    def derived_fb1(self):
        ...
        if np.all([p in self.columns for p in reqpars]):
            ...
            FB1ERRnew[idx] = np.sqrt(
                ...
                + (2.0 * FB0[idx] ** 3 * PBDOT[idx] * PBERR[idx] * 86400.0) ** 2
            )
    ...
    def derived_p1_i(self):
        ...
        idx = (
            np.isfinite(P1) & np.isfinite(P0) & np.isfinite(VTRANS) & np.isfinite(DIST)
        )
        P1I[idx] = (
            (P1[idx] / 1.0e-15)
            - VTRANS[idx] ** 2 * 1.0e10 * P0[idx] / (DIST[idx] * 3.086e6) / 2.9979e10
        ) * 1.0e-15
    ...
    def derived_b_lc(self):
        ...
        idx = (P1 > 0.0) & np.isfinite(P1) & np.isfinite(P0)
        BLC[idx] = 3.0e8 * np.sqrt(P1[idx]) * np.abs(P0[idx]) ** (-5.0 / 2.0)
    ...
    def derived_edot_i(self):
        ...
        idx = (P1_I > 0.0) & np.isfinite(P1_I) & np.isfinite(P0)
    ...
    def derived_edotd2(self):
        ...
        idx = (P0 > 0.0) & np.isfinite(P1) & np.isfinite(P0) & np.isfinite(DIST)
        EDOTD2[idx] = (
            4.0 * np.pi**2 * 1e45 * ((P1[idx] / P0[idx] ** 3) / DIST[idx] ** 2)
        )
    ...
    def derived_pmtot(self):
        ...
        if not np.all([p in self.columns for p in reqpars1]) and not np.all(
            [p in self.columns for p in reqpars2]
        ):
            return
        ...
        PMTOTERR = np.sqrt(
            ((PMLONG * PMLONGERR) ** 2 + (PMLAT * PMLATERR) ** 2)
            / (PMLONG**2 + PMLAT**2)
        )
    ...
    def derived_vtrans(self):
        ...
        VTRANS[idx] = (
            (PMTOT[idx] * np.pi / (1000.0 * 3600.0 * 180.0 * 365.25 * 86400.0))
            * 3.086e16
            * DIST[idx]
        )
    ...
    def derived_flux(self):
        ...
        idx = np.isfinite(S1400) & np.isfinite(S400) & (S1400 > 0.0) & (S400 > 0.0)
    ...
    def get_ephemeris(self, psr, precision=15, selected=False):
        ...
        mkl = (
            max([len(kn) for kn in variables]) + 2
        ...
        for varname, varval, varerr in zip(variables, values, errors):
            ...
            if isinstance(varval, float):
                ...
                outputdic["error"] = (
                    precstre.format(varerr) if varerr is not None else ""
                )
            else:
        ...
        if psrtype is not None:
            if isinstance(psrtype, list):
                ...
                for p in psrtype:
                    if not isinstance(p, str):
                        raise Exception(
                            "Non-string value '{}' found in pulsar type list".format(p)
                        )
            ...
            for p in list(self._query_psr_types):
                if p.upper() not in PSR_TYPE:
                    warnings.warn(
                        "Pulsar type '{}' is not recognised, no type will be required".format(
                            p
                        )
                    )
        ...
        if assoc is not None:
            if isinstance(assoc, list):
                ...
                for p in assoc:
                    if not isinstance(p, str):
                        raise Exception(
                            "Non-string value '{}' found in associations list".format(p)
                        )
            ...
            for p in list(self._query_assocs):
                if p.upper() not in PSR_ASSOC_TYPE:
                    warnings.warn(
                        "Pulsar association '{}' is not recognised, "
        ...
        if bincomp is not None:
            if isinstance(bincomp, list):
                ...
                for p in bincomp:
                    if not isinstance(p, str):
                        raise Exception(
                            "Non-string value '{}' found in binary "
            ...
            for p in list(self._query_bincomps):
                if p.upper() not in PSR_BINARY_TYPE:
                    warnings.warn(
                        "Pulsar binary companion '{}' is not recognised, "
                        "no type will be required".format(p)
                    )
    ...
    def ppdot(
        ...
        try:
            ...
            raise ImportError(
                "Cannot produce P-Pdot plot as Matplotlib is not available"
            )
        ...
        for stype in list(nshowtypes):
            if "ALL" == stype.upper():
                ...
                warnings.warn(
                    '"TYPE" {} is not recognised, so will not be '
        ...
        if excludeGCs:
            ...
            nongcidxs = np.flatnonzero(
                np.char.find(np.array(assocs.tolist(), dtype=str), "GC:") == -1
            )
        ...
        if periodlims is None:
            periodlims = [
                10 ** np.floor(np.min(np.log10(periods))),
                10.0 * int(1 + np.ceil(np.max(periods) / 10.0)),
            ]
        if pdotlims is None:
            pdotlims = [
                10 ** np.floor(np.min(np.log10(pdots))),
            ...
            if stype.upper() in PSR_TYPE + ["GC", "SNR"]:
                ...
                if thistype == "BINARY":
                    ...
                    typeidx = np.flatnonzero(
                        np.char.find(np.array(assocs.tolist(), dtype=str), thistype)
                        != -1
                    )
                else:
                    typeidx = np.flatnonzero(
                        np.char.find(np.array(types.tolist(), dtype=str), thistype)
                        != -1
                    )
                ...
                ax.legend(
                    handles.values(), handles.keys(), loc="upper left", numpoints=1
                )
        ...
        if showtau:
            ...
            for tauv in taus:
                ...
                if numv == 1.0:
                    ...
                    tlines[
                        r"${{{0:.1f}}}!\times\!10^{{{1:d}}}\,{{\rm yr}}$".format(
                            numv, taupow
                        )
                    ] = tline
        ...
        if showB:
            ...
            for B in Bs:
                ...
                if numv == 1.0:
                    ...
                    Blines[
                        r"${{{0:.1f}}}!\times\!10^{{{1:d}}}\,{{\rm G}}$".format(
                            numv, Bpow
                        )
                    ] = bline
        ...
        for l in tlines:
            _ = label_line(
                ax, tlines[l], l, color="k", fs=18, frachoffset=0.05, zorder=100
            )

        for l in Blines:
            _ = label_line(
                ax, Blines[l], l, color="k", fs=18, frachoffset=0.85, zorder=100
            )
    ...
    def gw_mass_quadrupole(self):
        ...
        q22 = h0_to_q22(table["H0_SD"], table["F0"], table["DIST"])
